	...
	for {
		...
		case <-timer.C:
			...
			if w.isRunning() && ((w.chainConfig.Ethash != nil) || (w.chainConfig.Clique != nil &&
				w.chainConfig.Clique.Period > 0) || (w.chainConfig.Femida != nil && w.chainConfig.Femida.Period > 0)) {
...
func (w *worker) mainLoop() {
	...
	for {
		...
		case ev := <-w.chainSideCh:
			...
			if _, ok := w.engine.(*femida.Femida); ok {
				continue
			}
		...
		case ev := <-w.txsCh:
			...
			if !w.isRunning() && w.current != nil {
				...
				if (w.chainConfig.Clique != nil && w.chainConfig.Clique.Period == 0) ||
					(w.chainConfig.Femida != nil && w.chainConfig.Femida.Period == 0) {
