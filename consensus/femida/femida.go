...
const (
	...
	allowedFutureBlockTime = time.Second
)
...
var (
	...
	systemContracts = map[common.Address]bool{
		...
		common.HexToAddress(systemcontracts.RewardContract):    true,
	}
)
...
func (p *Femida) VerifyHeader(chain consensus.ChainReader, header *types.Header, seal bool) error {
...
func (p *Femida) VerifyHeaders(chain consensus.ChainReader, headers []*types.Header, seals []bool) (chan<- struct{}, <-chan error) {
...
func (p *Femida) verifyHeader(chain consensus.ChainReader, header *types.Header, parents []*types.Header) error {
...
func (p *Femida) verifyCascadingFields(chain consensus.ChainReader, header *types.Header, parents []*types.Header) error {
...
func (p *Femida) snapshot(chain consensus.ChainReader, number uint64, hash common.Hash, parents []*types.Header) (*Snapshot, error) {
...
// from.
func (p *Femida) verifySeal(chain consensus.ChainReader, header *types.Header, parents []*types.Header) error {
...
func (p *Femida) Prepare(chain consensus.ChainReader, header *types.Header) error {
...
// rewards given.
func (p *Femida) Finalize(chain consensus.ChainReader, header *types.Header, state *state.StateDB, txs *[]*types.Transaction,
...
func (p *Femida) FinalizeAndAssemble(chain consensus.ChainReader, header *types.Header, state *state.StateDB,
...
func (p *Femida) Seal(chain consensus.ChainReader, block *types.Block, results chan<- *types.Block, stop <-chan struct{}) error {
	header := block.Header()
...
// current signer.
func (p *Femida) CalcDifficulty(chain consensus.ChainReader, time uint64, parent *types.Header) *big.Int {
...
func (p *Femida) APIs(chain consensus.ChainReader) []rpc.API {
	return []rpc.API{{
		Namespace: "femida",
		Version:   "1.0",
...
func (p *Femida) getCurrentValidators(blockHash common.Hash) ([]common.Address, error) {
	// block
...
func (p *Femida) distributeIncoming(val common.Address, state *state.StateDB, header *types.Header, chain core.ChainContext,
	...
	/**
	Here was part for system rewards, now all of this will be in one contract
	*/
...
func (p *Femida) initContract(state *state.StateDB, header *types.Header, chain core.ChainContext,
	...
	contracts := []string{
		...
		systemcontracts.RewardContract,
...
// get system message
func (p *Femida) getSystemMessage(from, toAddress common.Address, data []byte, value *big.Int) callmsg {
	return callmsg{
		ethereum.CallMsg{
			From: from,
...
func (p *Femida) applyTransaction(
	msg callmsg,
	state *state.StateDB,
	header *types.Header,
