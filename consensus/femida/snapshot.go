...
type Snapshot struct {
	config   *params.FemidaConfig // Consensus engine parameters to fine tune behavior
...
func loadSnapshot(config *params.FemidaConfig, sigCache *lru.ARCCache, db ethdb.Database, hash common.Hash, ethAPI *ethapi.PublicBlockChainAPI) (*Snapshot, error) {
	blob, err := db.Get(append([]byte("femida-"), hash[:]...))
	if err != nil {
		return nil, err
	}
	snap := new(Snapshot)
...
func (s *Snapshot) store(db ethdb.Database) error {
	...
	return db.Put(append([]byte("femida-"), s.Hash[:]...), blob)
}
