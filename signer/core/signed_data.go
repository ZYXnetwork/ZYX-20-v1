...
import (
	...
	"github.com/ethereum/go-ethereum/consensus/femida"
...
var (
	...
	ApplicationFemida = SigFormat{
		accounts.MimetypeFemida,
		0x03,
	}
	TextPlain = SigFormat{
...
func (api *SignerAPI) determineSignatureFormat(ctx context.Context, contentType string, addr common.MixedcaseAddress, data interface{}) (*SignDataRequest, bool, error) {
	...
	case ApplicationFemida.Mime:
		stringData, ok := data.(string)
		if !ok {
			return nil, useEthereumV, fmt.Errorf("input for %v must be an hex-encoded string", ApplicationFemida.Mime)
		}
		femidaData, err := hexutil.Decode(stringData)
		if err != nil {
			return nil, useEthereumV, err
		}
		header := &types.Header{}
		if err := rlp.DecodeBytes(femidaData, header); err != nil {
			return nil, useEthereumV, err
		}
		// The incoming femida header is already truncated, sent to us with a extradata already shortened
		if len(header.Extra) < 65 {
			// Need to add it back, to get a suitable length for hashing
			newExtra := make([]byte, len(header.Extra)+65)
			copy(newExtra, header.Extra)
		...
		// Get back the rlp data, encoded by us
		sighash, femidaRlp, err := femidaHeaderHashAndRlp(header, api.chainID)
		if err != nil {
			return nil, useEthereumV, err
		}
		messages := []*NameValueType{
			{
				Name:  "Femida header",
				Typ:   "femida",
				Value: fmt.Sprintf("femida header %d [0x%x]", header.Number, header.Hash()),
			},
		}
		// Femida uses V on the form 0 or 1
		useEthereumV = false
		req = &SignDataRequest{ContentType: mediaType, Rawdata: femidaRlp, Messages: messages, Hash: sighash}
...
func femidaHeaderHashAndRlp(header *types.Header, chainId *big.Int) (hash, rlp []byte, err error) {
	if len(header.Extra) < 65 {
		err = fmt.Errorf("clique header extradata too short, %d < 65", len(header.Extra))
		return
	}
	rlp = femida.FemidaRLP(header, chainId)
	hash = femida.SealHash(header, chainId).Bytes()
	return hash, rlp, err
}
