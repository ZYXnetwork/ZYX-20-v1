...
var (
	...
	MinervaFlag = cli.BoolFlag{
		Name:  "minerva",
		Usage: "Minerva network: pre-configured proof-of-authority test network",
	}
...
func MakeDataDir(ctx *cli.Context) string {
	if path := ctx.GlobalString(DataDirFlag.Name); path != "" {
		if ctx.GlobalBool(MinervaFlag.Name) {
			return filepath.Join(path, "minerva")
		}
		return path
	}
...
func setBootstrapNodes(ctx *cli.Context, cfg *p2p.Config) {
	...
	case ctx.GlobalBool(MinervaFlag.Name):
		urls = params.MinervaBootnodes
...
func setDataDir(ctx *cli.Context, cfg *node.Config) {
	...
	case ctx.GlobalBool(MinervaFlag.Name) && cfg.DataDir == node.DefaultDataDir():
		cfg.DataDir = filepath.Join(node.DefaultDataDir(), "minerva")
	}
}
...
func SetEthConfig(ctx *cli.Context, stack *node.Node, cfg *eth.Config) {
	...
	CheckExclusive(ctx, DeveloperFlag, MinervaFlag)
	...
	case ctx.GlobalBool(MinervaFlag.Name):
		...
		cfg.Genesis = core.DefaultMinervaGenesisBlock()
		//setDNSDiscoveryDefaults(cfg, params.KnownDNSNetworks[params.MinervaGenesisHash])
	case ctx.GlobalBool(DeveloperFlag.Name):
		if !ctx.GlobalIsSet(NetworkIdFlag.Name) {
			cfg.NetworkId = 1337
		}
...
func MakeGenesis(ctx *cli.Context) *core.Genesis {
	...
	switch {
	case ctx.GlobalBool(MinervaFlag.Name):
		genesis = core.DefaultMinervaGenesisBlock()
	case ctx.GlobalBool(DeveloperFlag.Name):
