...
func InspectDatabase(db ethdb.Database) error {
	...
	var (
		...
		femidaSnapsSize common.StorageSize
	...
	for it.Next() {
		...
		case bytes.HasPrefix(key, []byte("femida-")) && len(key) == 7+common.HashLength:
			femidaSnapsSize += size
	...
	stats := [][]string{
		...
		{"Key-Value store", "Femida snapshots", femidaSnapsSize.String()},
