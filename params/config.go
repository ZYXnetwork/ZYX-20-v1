...
var (
	...
	// Zyx testnets
	MinervaGenesisHash    = common.HexToHash("0xd4e56740f876aef8c010b86a40d5f56745a118d0906a34e69aec8c0db1cb8fa3")
)
...
var (
	...
	MinervaChainConfig = &ChainConfig{
...
type ChainConfig struct {
	...
	Femida *FemidaConfig `json:"femida,omitempty" toml:",omitempty"`
}
...
// FemidaConfig is the consensus engine configs for proof-of-staked-authority based sealing.
type FemidaConfig struct {
	Period uint64 `json:"period"` // Number of seconds between blocks to enforce
	Epoch  uint64 `json:"epoch"`  // Epoch length to update validatorSet
}

// String implements the stringer interface, returning the consensus engine details.
func (b *FemidaConfig) String() string {
	return "femida"
}
...
func (c *ChainConfig) String() string {
	...
	return fmt.Sprintf("{ChainID: %v Homestead: %v DAO: %v DAOSupport: %v EIP150: %v EIP155: %v EIP158: %v Byzantium: %v Constantinople: %v Petersburg: %v Istanbul: %v, Muir Glacier: %v, Niels: %v, MirrorSync: %v, Engine: %v}",
		c.ChainID,
		c.HomesteadBlock,
		c.DAOForkBlock,
		c.DAOForkSupport,
